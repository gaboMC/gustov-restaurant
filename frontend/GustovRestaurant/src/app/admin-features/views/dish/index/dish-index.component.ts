...
export class DishIndexComponent {
  ...
  openDishForm(dishId: number | null = null): void{
    ...
    deleteSoft(dishId: number) {
      this.#dishService.deleteSoft$(dishId, this.#destroyRef)
        .pipe(
          switchMap((response) => {
            return this.#showDishes$();
          })
        ...
        .subscribe({
          next: () => {
            console.log('Lista actualizada después de la eliminación.');
          },
          error: (e) => console.error('Error al eliminar o recargar los datos:', e)
        });
    }
